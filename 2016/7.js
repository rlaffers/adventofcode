...
// check if given 4 chars are an ABBA sequence
function isAbba(char1, char2, char3, char4) {
...
for (var i=0; i < input.length; i++) {
    ...
    for (var j=0; j < line.length - 3; j++) {
        ...
        hasAbbaNow = isAbba(char, line[j+1], line[j+2], line[j+3]);
        
        if (withinBracket && hasAbbaNow) {
            // no tls
            notls.push(line);
            // go to another line
            break;
        } else if (!withinBracket && hasAbbaNow) {
            // can be tls, need to continue till the end of line
            hadAbba = true;
        } else {
            // no abba now
        }
...
console.log("TLS supported in " + tls.length + " IPs");


// checks if 3 chars are ABA sequence
function isAba(char1, char2, char3) {
...
// checks if given 3 chars are in any of the hypernet groups in the reverse sequence
function isInHypernet(char1, char2, char3, hypers) {
    var seq = char2 + char1 + char2;
    var h;
    for (var i = hypers.length - 1; i >= 0; i--) {
        h = hypers[i];
        if (h.indexOf(seq) !== -1) {
            console.log("sequence " + seq + " is in hypergroup ", h);
            return true;
        }
    }
    return false;
...
for (var i=0; i < input.length; i++) {
    ...
    for (var j=0; j < line.length - 2; j++) {
        ...
        if (isAba(char, line[j+1], line[j+2])) {
            // is this aba in hypernet brackets?
            if (isInHypernet(char, line[j+1], line[j+2], hyper)) {
                ssl.push(line);
                break;
            }
        }
...
//console.log("ssl", ssl);
console.log("ssl count", ssl.length);
