...
//input = [
    ...
    //// for second part of the puzzle only
...
var matches, 
    ...
    decoy = []
    ;
console.log("input has " + input.length + ' items');
for (var i=0; i < input.length; i++) {
    //item = input[i].replace(/-/g, '');
    item = input[i];
    matches = item.match(re);
    //console.log(matches);
    if (!matches) {
        throw 'invalid input:' + input[i];
    }
    var charCount = {};
    var char;
    ...
    for (var key in charCount) {
        sortable.push([key, charCount[key]]);
    }
...
// count the result
//console.log("real rooms", real);
var sum = 0;
for (var i=0; i < real.length; i++) {
    sum += parseInt(real[i][2]);
}
console.log('Real rooms sector count=', sum);

var revealedRooms = [];
for (var i=0; i < real.length; i++) {
    var name = real[i][1];
    var salt = parseInt(real[i][2]);
    unencrypted = '';
    for (var j=0; j < name.length; j++) {
        unencrypted += rotateChar(name[j], salt);
    }
    revealedRooms[unencrypted] = salt;
}
//console.log(revealedRooms);

var re2 = /north/i;
for (var name in revealedRooms) {
    if (name.match(re2)) {
        console.log(name + ' has sector ID = ', revealedRooms[name]);
    }
}



function rotateChar(char, n) {
    if (char === '-') {
        return ' ';
    }
    var point = char.charCodeAt(0);
    var shifted, shiftedPoint;

    // a = 97, z = 122
    shiftedPoint = point + n;
    while (shiftedPoint > 122) {
        shiftedPoint = shiftedPoint - 26;
    }
    shifted = String.fromCharCode(shiftedPoint);
    //console.log(n, 'shifted ' + char + ' (' + point + ') to ' + shifted);
    return shifted;
}
