...
func main() {
	...
	// find viable pairs
	pairs := findPairs(nodes)
	fmt.Printf("pairs = %d\n", len(pairs))

	fmt.Printf("looking for data at: %d, 0\n", len(nodes)-1)

	// print current situation
	printNodes(nodes)
}

func findPairs(nodes map[int]map[int]*Node) [][]*Node {
	pairs := [][]*Node{}
	for x, row := range nodes {
		for y, nodeA := range row {
			if nodeA.used < 1 {
				// this node is empty, never will have a viable pair
				dbg("node %d,%d is empty so not viable", x, y)
				continue
			}
			pairableNodes := getViableNodes(nodes, nodeA)
			for _, nodeB := range pairableNodes {
				pairs = append(pairs, []*Node{nodeA, nodeB})
			}
		}
	}
	return pairs
...
func getViableNodes(nodes map[int]map[int]*Node, nodeA *Node) []*Node {
	viable := []*Node{}
	for _, row := range nodes {
		for _, nodeB := range row {
			if nodeA.x == nodeB.x && nodeA.y == nodeB.y {
				// not viable, the same node
				dbg("node %d,%d is the same as %d, %d", nodeB.x, nodeB.y, nodeA.x, nodeA.y)
				continue
			}
			// will fit?
			if nodeA.used > nodeB.avail {
				dbg("data on %d,%d (%d) will not fit on %d,%d (%d)", nodeA.x, nodeA.y, nodeA.used, nodeB.x, nodeB.y, nodeB.avail)
				continue
			}
			// is viable
			viable = append(viable, nodeB)
		}
	}
	dbg("viable nodes for nodeA %d,%d (%d): %v", nodeA.x, nodeA.y, nodeA.used, viable)
	return viable
}

func parseInput(input []string) map[int]map[int]*Node {
	re := regexp.MustCompile("x([0-9]+)-y([0-9]+)\\s+([0-9]+)T\\s+([0-9]+)T\\s+([0-9]+)T")
	var matched []string
	nodes := map[int]map[int]*Node{}
	var x, y, used, avail int
	var err error
	var row map[int]*Node
	var ok bool
	for _, item := range input {
		matched = re.FindStringSubmatch(item)
		if len(matched) < 1 {
			panic(fmt.Errorf("invalid item %s", item))
		}
		...
		used, err = strconv.Atoi(matched[4])
		if err != nil {
			panic(err)
		}

		avail, err = strconv.Atoi(matched[5])
		if err != nil {
			panic(err)
		}
...
func printRow(row map[int]*Node) {
	for col, node := range row {
		fmt.Printf("%d node = %+v\n", col, *node)
	}
}

func printNodes(nodes map[int]map[int]*Node) {
	fmt.Println("Printing nodes (Used/Available)")
	for _, row := range nodes {
		for _, node := range row {
			fmt.Printf("%d/%d  ", node.used, node.avail)
		}
		fmt.Printf("\n")
	}
}
