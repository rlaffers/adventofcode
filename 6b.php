...
foreach ($instructions as $i => $ins) {
    if (!preg_match('/^(turn off|turn on|toggle)\s([0-9]+),([0-9]+)\sthrough\s([0-9]+),([0-9]+)$/', $ins, $match)) {
        die('error matching line ' . $i);
    }
    $x1 = $match[2];
    $y1 = $match[3];
    $x2 = $match[4];
...
$luminosity = array_sum($lit);
echo "Finished at: " . round(microtime(true) - $start, 3) . " sec\n";
echo "Total luminosity: $luminosity";
