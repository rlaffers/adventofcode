...
import { gt, add, length, filter, reduce, reduced, range, xprod, trim, map, compose, split } from 'ramda'
// TODO cleanup
const input = readFileSync('./6_input').toString().trim()

const parseInput = compose(
  map(map(compose(Number, trim))),
  map(split(',')),
  split('\n'),
)
...
// find min, max x, y
const extremeCoords = aggregator => ([minX, minY], [x, y]) => [
  aggregator(minX, x),
  aggregator(minY, y),
]
...
const pointOwners = compose(
  res => new Map(res),
  map(findClosestDestination(destinations)),
)(grid)
...
const solution2 = compose(
  console.log,
  length,
  filter(isPointSumDistanceBelow1000(destinations)),
)
solution2(grid)
