...
import { invertObj, compose, reduce, __, replace, min, map, length, applyTo, transduce } from 'ramda'
...
const subpolymerList = (regex, units) =>
  units.map(regex).map(re => replace(re, ''))
