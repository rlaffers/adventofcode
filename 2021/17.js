...
const speedToTravelDistance = (n) => (n * (n + 1)) / 2

// PART 1
const solver1 = S.pipe([maxUpwardSpeed, speedToTravelDistance])

// --------------------------------------------------------------------
// PART 2
const findXSpeedLimits = ({ x1, x2 }) => {
  const max = x2 + 1
  let min = 0
  while (speedToTravelDistance(min) < x1) {
    min += 1
  }
  return [min, max]
}

const findYSpeedLimits = ({ y1, y2 }) => {
...
const solver2 = S.ap(tryAllSpeeds)(
  S.lift2(R.pair)(findXSpeedLimits)(findYSpeedLimits),
)
