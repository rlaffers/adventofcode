...
var reDirect = /^value (\d+) goes to bot (\d+)$/i;
var re = /^bot (\d+) gives low to (bot|output) (\d+) and high to (bot|output) (\d+)$/i;

var instruction, matched;
for (var i=0; i < input.length; i++) {
    instruction = input[i];
    //console.log("parsing '" + instruction + "'");
    switch (true) {
        case (matched = instruction.match(reDirect)) !== null:
            //console.log('1', match);

            // no need to save, just execute
            //evaluateSimple(matched, bots);
            break;

        case (matched = instruction.match(re)) !== null:
            saveInstruction(instructions, matched, bots, outputs);
            break;

        default:
            console.error('invalid instruction', instruction, reDirect);
            process.exit(1);
    }
}
...
for (var i=0; i < input.length; i++) {
    ...
    switch (true) {
        ...
        case (matched = instruction.match(re)) !== null:
            //evaluate(findInstruction(instructions, matched[1]), '');
            break;
...
function mustEvaluate(bots) {
    // must evaluate if at least one bot contains more than 1 value
    for (var bot in bots) {
...
// saves given instruction for later use
function saveInstruction(instructions, matched, bots, outputs) {
    var bot = matched[1],
        typeLow = matched[2],
        typeHigh = matched[4],
        numLow = matched[3],
        ...
        refHigh;
    if (typeLow === 'bot') {
        if (bots[numLow] === undefined) {
            bots[numLow] = [];
        }
        refLow = bots[numLow];
    } else {
        if (outputs[numLow] === undefined) {
            outputs[numLow] = [];
        }
        refLow = outputs[numLow];
    }

    if (typeHigh === 'bot') {
        if (bots[numHigh] === undefined) {
            bots[numHigh] = [];
        }
        refHigh = bots[numHigh];
    } else {
        if (outputs[numHigh] === undefined) {
            outputs[numHigh] = [];
        }
        refHigh = outputs[numHigh];
    }
...
// evaluates given parsed instruction
function evaluate(instruction, tab) {
    //console.log("evaluating", instruction);
    if (bots[instruction.source] === undefined) {
        // no source
        bots[instruction.source] = [];
    }
    var source = bots[instruction.source];
    if (source.length < 2) {
        // source not not full yet
        //console.log("source" + instruction.source + " not full yet");
        return;
    }
    source = source.sort();

    console.log(tab + "-> evaluating", instruction.str);
    tab = ' ' + tab;
    console.log(tab + "source:", source, '-->', instruction.lowDest.ref, ' and ', instruction.highDest.ref);

    //if (source[0] == 61 || source[1] == 61 || source[0] == 17 || source[1] == 17) {
        //console.log("!!!go check", source, instruction.lowDest.ref, instruction.highDest.ref);
    //}
    if (doTest(source)) {
        console.log(tab + "bots", bots);
        throw 'responsible bot: ' + instruction.source;
    } else {
        console.log(tab + instruction.source + " is not responsible", source);
    }

    instruction.lowDest.ref.push(source[0]);
    instruction.highDest.ref.push(source[1]);
    console.log(tab + "bot " + instruction.lowDest.num + " after push", instruction.lowDest.ref);
    console.log(tab + "bot " + instruction.highDest.num + " after push", instruction.highDest.ref);
    
    // take the first out from source
    //source.splice(0, 1);
    source.splice(0, source.length); // concatenate the array

    // if any of the destinations was a bot, let's evaluate instruction for that bot
    if (instruction.lowDest.type === 'bot') {
        if (doTest(instruction.lowDest.ref)) {
            //console.log(tab + "bots", bots);
            console.log('done: ' + instruction.lowDest.num);
            process.exit();
        }
        evaluate(findInstruction(instructions, instruction.lowDest.num), tab);
    }

    //instruction.highDest.ref.push(source[0]);
    //console.log(tab + "bot " + instruction.highDest.num + " after push", instruction.highDest.ref);
    source.splice(0, source.length); // concatenate the array

    if (instruction.highDest.type === 'bot') {
        if (doTest(instruction.highDest.ref)) {
            console.log(tab + "bots", bots);
            throw 'done: ' + instruction.highDest.num;
        }
        evaluate(findInstruction(instructions, instruction.highDest.num), tab);
    }
    //console.log(tab + "<-");
}
...
function evaluateSimple(instruction, bots) {
    var botNum = instruction[2],
        value = instruction[1];
    if (bots[botNum] === undefined) {
        bots[botNum] = [];
    }
    bots[botNum].push(parseInt(value));
    bots[botNum] = bots[botNum].sort();
    printNonEmptyBots(bots);
    evaluate(findInstruction(instructions, botNum), '|');
}

function printNonEmptyBots(bots) {
    var bot, printable = { };
    for (var i in bots) {
        bot = bots[i];
        if (bot.length > 0) {
...
function doTest(bot) {
    ...
    if ((bot[0] == val1 && bot[1] == val2) || (bot[0] == val2 && bot[1] == val1)) {
        console.log("!!!responsible bot:", bot);
        return true;
    }
    //console.log("not the resonsible bot", bot);
    return false;
}

function findInstruction(instructions, botNum) {
    var ins;
    for (var i=0; i < instructions.length; i++) {
        ins = instructions[i];
        if (ins.source == botNum) {
            return ins;
        }
    }
    throw 'no instructions found for bot ' + botNum;
}
